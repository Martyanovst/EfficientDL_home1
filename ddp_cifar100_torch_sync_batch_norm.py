...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.bn1 = nn.SyncBatchNorm(128)  # to be replaced with SyncBatchNorm
...
if __name__ == "__main__":
    ...
    for batch_size in [32, 64]:
        for bn_size in [256, 512, 1024]:
            start_epoch = time.time()
            torch.cuda.synchronize()
            run_training(local_rank, size)
            end_epoch = time.time()
            print(f'Batch size: {batch_size}; batch-norm size: {bn_size} Elapsed time :{end_epoch - start_epoch}')